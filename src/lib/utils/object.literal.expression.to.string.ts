...
const objectLiteralExpressionToString: (expression: ObjectLiteralExpression) => any = (
  ...
  try {
    ...
    expression.properties.forEach(
      (propertyAssignment: PropertyAssignment): any => {
        obj[IdentifierToString(propertyAssignment.name as Identifier)] = InitializerToString(
...
export { objectLiteralExpressionToString as ObjectLiteralExpressionToString }
